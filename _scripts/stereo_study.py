...
def stereo_3d_study():
    ...
    for noise, n_landmarks in itertools.product(noise_list, n_landmark_list):
        print(f"============= noise {noise} / n_landmarks {n_landmarks} ================")
        np.random.seed(seed)
        ...
        learner = Learner(
            lifter=lifter, variable_list=lifter.variable_list, apply_templates=False, noise=noise
        )
        data_here = learner.run(verbose=True, use_known=False, plot=False, tightness=tightness)[0]
        dcost = learner.dual_costs[-1]
        qcost = learner.solver_vars["qcqp_cost"]
        data_here.update({"value": dcost, "cost": "dual", "noise":noise, "n_landmarks":n_landmarks})
        data.append(deepcopy(data_here))
        data_here.update({"value": qcost, "cost": "qcqp", "noise":noise, "n_landmarks":n_landmarks})
        data.append(deepcopy(data_here))
        data_here.update({"value": abs(qcost - dcost) / qcost, "cost": "ratio", "noise":noise, "n_landmarks":n_landmarks})
        data.append(deepcopy(data_here))
    ...
    for [cost, noise], df_ in df.groupby(["cost", "noise"]):
        ...
        ax.scatter(df_.n_landmarks, df_.value, color=colors[noise], marker=markers[cost], label=label)
    ax.set_yscale("log")
    ...
    for ratio in ["ratio", "ratio2", "ratio3"]:
        fig, ax = plt.subplots()
        fig.set_size_inches(5, 5)
        sns.scatterplot(pt, ax=ax, x="noise", hue="n_landmarks", color=colors, y=ratio, palette="tab10")
...
if __name__ == "__main__":
    ...
    #stereo_tightness(d=3, n_landmarks=5)
